...
- сохраняет Telegram ID, username, отображаемое имя и только видимый самому аккаунту телефон;
- фиксирует согласие на звонок отдельным событием;
...
search profile
    ...
    -> signal
    -> provisional lead
    -> human review + consent event
    -> CSV / CRM handoff
```

Публичное сообщение не считается согласием на телефонный звонок. Поэтому номер
скрывается из CSV до фиксации согласия в панели или API.
...
CSV лидов содержит исходное сообщение и permalink. Телефон выгружается только
если для лида есть актуальное положительное событие согласия на звонок.
...
- MTProto видит только то, что доступно авторизованному аккаунту, и не раскрывает
  скрытые номера телефонов.
- Автоматическое вступление в группы намеренно отсутствует.
- Коннектор к `@AXejEB_hAxg_bot` не включён: у источника данных не подтверждены
  происхождение данных, правовое основание, политика хранения и удаления.
- Перед production-обзвоном необходима отдельная проверка правил Telegram,
  применимых privacy/telemarketing требований и собственного основания обработки.
